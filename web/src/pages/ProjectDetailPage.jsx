...
import {
  ...
  getReadme, uploadExcel, chatExplain, chatSuggest,
  updateStudent, deleteStudent, templateUrl,
} from '../api';
...
export default function ProjectDetailPage() {
  ...
  const [editForm, setEditForm] = useState({ title: '', github: '', guide: '' });
  ...
  function openEdit() {
    setEditForm({ title: project.title || '', github: project.github || '', guide: project.guide || '' });
    setEditErr('');
    setEditOpen(true);
  }
  ...
  async function handleFileChange(e) {
    ...
    try {
      const result = await uploadExcel(projectId, file);
    ...
    <>
        ...
        {breadSem && (
          <>
            ...
            <Link to={`/semesters/${breadSem.semester_id}/courses`}
              state={{ semLabel: `Sem ${breadSem.sem_number}`, batchName: breadBatch?.batch_name }}>
        ...
        {breadCourse && (
          <>
            ...
            <Link to={`/courses/${breadCourse.course_id}/projects`}
              state={{ courseName: breadCourse.course_name, semLabel: `Sem ${breadSem?.sem_number}`, batchName: breadBatch?.batch_name }}>
      ...
      <div className="page-header-row" style={{ marginBottom: 28 }}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.8rem', fontWeight: 800 }}>
          {p.title || <span style={{ color: 'var(--text-muted)' }}>Untitled Project</span>}
        </h2>
        {canEdit && (
      ...
      <div className="detail-grid">
        ...
        <div>
          <div className="brutal-card">
            ...
            <div className="detail-field">
              <div className="field-key">Guide / Mentor</div>
              <div className={`field-val${p.guide ? '' : ' muted'}`}>{p.guide || 'Not set'}</div>
            </div>
            {breadCourse && (
              <div className="detail-field">
          ...
          <div style={{ marginTop: 20 }}>
            ...
            {p.students?.length > 0 ? (
              ...
              <div className="brutal-card dark" style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.85rem', fontStyle: 'italic' }}>
                No team members — upload a sheet to add them
              </div>
            )}
          </div>

          {/* Upload — coordinators/admin only; students can download template */}
          <div style={{ marginTop: 20 }}>
            <div className="label text-accent" style={{ marginBottom: 10 }}>Team Sheet (XLSX)</div>
            {uploadErr && <div className="alert error">{uploadErr}</div>}
            {uploadResult && (
              <div className="alert success">
                ✓ {uploadResult.students_inserted} member(s) added
                {uploadResult.partial_slots_skipped > 0 &&
                  `, ${uploadResult.partial_slots_skipped} incomplete slot(s) skipped`}
              </div>
            )}
            {canEdit && (
              <>
                <div className="upload-zone" onClick={() => !uploading && fileRef.current?.click()}
                  style={{ padding: '24px', cursor: uploading ? 'default' : 'pointer' }}>
                  {uploading
                    ? <><Spinner /><p style={{ marginTop: 8 }}>Uploading…</p></>
                    : <><div className="upload-icon">⊡</div><p>Click to pick .xlsx file</p></>
                  }
                </div>
                <input ref={fileRef} type="file" accept=".xlsx" style={{ display: 'none' }} onChange={handleFileChange} />
              </>
            )}
            <div style={{ marginTop: 10, textAlign: canEdit ? 'right' : 'left' }}>
              <a
                href={templateUrl()}
                target="_blank"
                rel="noreferrer"
                className="pill-btn outline"
                style={{ display: 'inline-flex', fontSize: '0.65rem' }}
              >
                ↓ Download Template
              </a>
            </div>
        ...
        <div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {readme && (
              readme.found
                ? <div className="readme-box">{readme.content}</div>
            ...
            <div>
              ...
              {suggestions.related?.length > 0 && (
                <div style={{ marginBottom: 12 }}>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'var(--text-muted)', marginBottom: 6 }}>
                    INSPIRED BY
                  </div>
      ...
      <Modal open={editOpen} onClose={() => setEditOpen(false)} title="Edit Project">
        <form onSubmit={handleEdit}>
          ...
          <input className="text-input" value={editForm.guide}
            onChange={e => setEditForm(f => ({ ...f, guide: e.target.value }))} />
      ...
      <Modal open={deleteConfirm} onClose={() => setDeleteConfirm(false)} title="Delete Project">
        <p style={{ color: 'var(--text-muted)', marginBottom: 8 }}>
          This will permanently delete <strong style={{ color: 'var(--text)' }}>{p.title || 'this project'}</strong> and all its students.
        </p>
        <div className="modal-actions">
